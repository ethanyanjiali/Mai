...
def collate_fn(batch, tokenizer, block_size, device, input_key):
    out_batch = tokenizer(
        list(batch[input_key]),
        padding="max_length",
...
def parse_args():
    ...
    parser.add_argument(
        "--num_devices", "-nd", type=int, default=4, help="Number of devices to use."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--model_name", default="meta_llama/Llama_2_7b_chat_hf", type=str
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="If passed, will enable parameter efficient fine_tuning with LoRA ("
    ...
    parser.add_argument(
        ...
        type=str,
        help="Anyscale artifact storage as trainer storage path root",
    )

    parser.add_argument(
        "--cpu_per_worker",
        type=int, default=2, help="Assigned CPU cores per worker."
    )

    parser.add_argument(
        "--gpu_per_worker",
        type=int, default=1, help="Assigned GPU devices per worker."
    )
...
def main():
    ...
    os.environ["RAY_AIR_LOCAL_CACHE_DIR"] = "/tmp/ray"
    ...
    artifact_storage = args.anyscale_artifact_storage
    user_name = re.sub(r"\s+", "__", os.environ.get("ANYSCALE_USERNAME", "user"))
    storage_path = os.path.join(artifact_storage, user_name, "ft_llms_with_deepspeed", args.model_name)
    ...
    trainer = TorchTrainer(
        ...
        scaling_config=train.ScalingConfig(
            ...
            use_gpu=True,
            resources_per_worker={"GPU": args.gpu_per_worker, "CPU": args.cpu_per_worker},
        ),
